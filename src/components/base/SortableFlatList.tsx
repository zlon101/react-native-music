...
interface ISortableFlatListProps<T> {
  ...
  // 高度
  itemHeight: number;
  itemJustifyContent?: 'flex-start' | 'flex-end' | 'center' | 'space-between' | 'space-around' | 'space-evenly';
...
export default function SortableFlatList<T extends any = any>(props: ISortableFlatListProps<T>) {
  ...
  function scrollToTarget(forceScroll = false) {
    ...
    const nextTarget =
      Math.sign(direction.value) * Math.max(Math.abs(direction.value), 0.3) * 300 + contentOffsetYRef.current;
    // 当前到极限了
    if (
      (contentOffsetYRef.current <= 2 && nextTarget < contentOffsetYRef.current) ||
      (contentOffsetYRef.current >= listContentHeight - (layoutRef.current?.height ?? 0) - 2 &&
  ...
  return (
    <View style={globalStyle.fwflex1}>
      ...
      <FlashList
        ...
        onTouchMove={e => {
          if (activeRef.current !== -1) {
            offsetRef.current = e.nativeEvent.pageY - (marginTop ?? layoutRef.current?.y ?? 0) - itemHeight / 2;

            if (offsetRef.current < 0) {
              offsetRef.current = 0;
            } else if (offsetRef.current > (layoutRef.current?.height ?? 0) - itemHeight) {
              offsetRef.current = (layoutRef.current?.height ?? 0) - itemHeight;
            }
            ...
            if (offsetRef.current < itemHeight * 2) {
              // 上滑
              direction.value = offsetRef.current / itemHeight / 2 - 1;
            } else if (offsetRef.current > (layoutRef.current?.height ?? 0) - 3 * itemHeight) {
              // 下滑
              direction.value =
                (offsetRef.current - (layoutRef.current?.height ?? 0) + 3 * itemHeight) / itemHeight / 2;
            } else {
              // 不滑动
              direction.value = 0;
            }
        ...
        onTouchEnd={e => {
          if (activeRef.current !== -1) {
            ...
            if (contentOffsetYRef.current !== -1) {
              index = Math.round((contentOffsetYRef.current + offsetRef.current) / itemHeight);
            } else {
              // 拖动的距离
              index =
                activeRef.current +
                Math.round((e.nativeEvent.pageY - initDragPageY.current + initDragLocationY.current) / itemHeight);
            }
            index = Math.min(data.length, Math.max(index, 0));
            // from: activeRef.current to: index
            if (activeRef.current !== index) {
              let nData = _data.slice(0, activeRef.current).concat(_data.slice(activeRef.current + 1));
        ...
        onScroll={e => {
          ...
          if (activeRef.current !== -1 && Math.abs(contentOffsetYRef.current - targetOffsetYRef.current) < 2) {
            scrollToTarget(true);
          }
        }}
...
const FakeFlatListItem = forwardRef(function (
  props: Pick<ISortableFlatListItemProps, 'itemHeight' | 'renderItem' | 'item' | 'itemJustifyContent'> & {
