...
export default function (props: IPanelBaseProps) {
  ...
  const useAnimatedBase = useMemo(() => (orientation === 'horizonal' ? rpx(750) : height), [orientation]);
  ...
  useEffect(() => {
    ...
    const listenerSubscription = DeviceEventEmitter.addListener('hidePanel', (callback?: () => void) => {
      if (callback) {
    ...
    return {
      transform: [
        orientation === 'vertical'
          ? {
              translateY: withTiming((1 - snapPoint.value) * useAnimatedBase, timingConfig),
            }
          : {
              translateX: withTiming((1 - snapPoint.value) * useAnimatedBase, timingConfig),
            },
      ],
    };
  }, [orientation]);
    ...
    <Animated.View
      style={[
        style.wrapper,
        {
          ...
          height: orientation === 'horizonal' ? vh(100) - safeAreaInsets.top : height,
        },
        panelAnimated,
      ]}>
      {renderBody(loading)}
  ...
  return (
    <>
      ...
      {keyboardAvoidBehavior === 'none' ? (
        ...
        <KeyboardAvoidingView style={style.kbContainer} behavior={keyboardAvoidBehavior || 'position'}>
          {panelBody}
        </KeyboardAvoidingView>
      )}
    </>
