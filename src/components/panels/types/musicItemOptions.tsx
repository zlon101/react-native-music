...
import { EDeviceEvents, localMusicSheetId, musicHistorySheetId } from '@/constants/commonConst';
...
export default function MusicItemOptions(props: IMusicItemOptionsProps) {
  ...
  const options = [
    ...
    {
      icon: 'link-variant',
      title: associatedLrc ? `已关联歌词 ${associatedLrc.platform}@${associatedLrc.id}` : '关联歌词',
      onPress: async () => {
        if (Config.get('setting.basic.associateLyricType') === 'input') {
      ...
      renderBody={() => (
        <>
          <View style={style.header}>
            <FastImage style={style.artwork} uri={musicItem?.artwork} emptySrc={ImgAsset.albumDefault} />
              ...
              renderItem={({ item }) =>
                item.show !== false ? (
                  <ListItem withHorizonalPadding heightType="small" onPress={item.onPress}>
                    <ListItem.ListItemIcon width={rpx(48)} icon={item.icon} iconSize={iconSizeConst.small} />
