...
export default function useSearchLrc() {
   ...
   * pluginHash: 搜索条件
   */
  const search = useCallback(async function (query?: string, queryPage?: number, pluginHash?: string) {
    /** 如果没有指定插件，就用所有插件搜索 */
    ...
    plugins.forEach(async plugin => {
      ...
      // 是否是一次新的搜索
      const newSearch = query || prevPluginResult?.page === undefined || queryPage === 1;

      // 本次搜索关键词
      currentQueryRef.current = query = query ?? searchResultStore.getValue().query ?? '';
        ...
        searchResultStore.setValue(
          produce(draft => {
            ...
            prevMediaResult[_hash] = {
              state: newSearch ? RequestStateCode.PENDING_FP : RequestStateCode.PENDING,
              // @ts-ignore
        ...
        searchResultStore.setValue(
          produce(draft => {
            ...
            prevMediaResult[_hash] = {
              ...
              page,
              data: newSearch ? currResult : (prevPluginResult.data ?? []).concat(currResult),
            };
            return draft;
          }),
        );
