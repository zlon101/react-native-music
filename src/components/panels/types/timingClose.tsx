...
function CountDownHeader() {
  ...
  return (
    <View style={style.header}>
      <ThemeText fontWeight="medium">{countDown === null ? '定时关闭' : `倒计时 ${timeformat(countDown)}`}</ThemeText>
      <ThemeSwitch
        value={countDown !== null}
        onValueChange={val => {
          if (val === true) {
            if (shortCutValue) {
              setTimingClose(getDeadlineTimestamp(0, shortCutValue));
              return;
            }
            setTimingClose(getDeadlineTimestamp(selectedHour, selectedMin));
          } else {
            setTimingClose(null);
          }
        }}
...
function NumScrollView() {
  ...
  const onMinMomentumScrollEnd = useCallback(
    debounce(
      () => {
        selectedMin = `${(minOffsetYRef.current / ITEM_HEIGHT).toFixed(0)}`;
      ...
      <View style={numScrollStyles.listWrapper}>
        <FlatList
          ...
          onMomentumScrollEnd={e => {
            selectedHour = `${(e.nativeEvent.contentOffset.y / ITEM_HEIGHT).toFixed(0)}`;
          }}
          onScrollEndDrag={e => {
            selectedHour = `${(e.nativeEvent.contentOffset.y / ITEM_HEIGHT).toFixed(0)}`;
          }}
          ...
          data={hours}
          renderItem={({ item }) => <ThemeText style={numScrollStyles.item}>{item} 时</ThemeText>}
        />
        <FlatList
          ref={ref => {
          ...
          onScrollEndDrag={e => {
            selectedMin = `${(e.nativeEvent.contentOffset.y / ITEM_HEIGHT).toFixed(0)}`;
          }}
          data={minutes}
          renderItem={({ item }) => <ThemeText style={numScrollStyles.item}>{item} 分</ThemeText>}
        />
        <View style={numScrollStyles.topBanner} pointerEvents={'none'} />
        <View style={numScrollStyles.bottomBanner} pointerEvents={'none'} />
      </View>
    </View>
  );
}
                ...
                <TouchableOpacity
                  ...
                  onPress={() => {
                    if (selectedShortCut !== time) {
                      ...
                      if (time) {
                        setTimingClose(Date.now() + time * 60000);
                      } else {
                        setTimingClose(Date.now() + ((+selectedHour || 0) * 60 + (+selectedMin || 1)) * 60000);
                      }
                    }
                  }}>
                  <View
                    ...
                    <ThemeText fontColor={selectedShortCut === time ? 'appBarText' : 'text'}>
                      {time ? `${time}min` : '自定义'}
                    </ThemeText>
                  </View>
                </TouchableOpacity>
