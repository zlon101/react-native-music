...
export default function AboutSetting() {
  ...
  return (
    <View
      ...
      <View style={[style.header, orientation === 'horizonal' ? style.horizonalSize : null]}>
        <TouchableOpacity
          onPress={() => {
            checkAndShowResult(true);
          }}>
          <Image source={ImgAsset.logo} style={style.image} resizeMode="contain" />
        </TouchableOpacity>
        <ThemeText style={style.margin}>当前版本: {deviceInfoModule.getVersion()} (点击logo检查更新)</ThemeText>
      </View>
      <ScrollView contentContainerStyle={style.scrollViewContainer} style={style.scrollView}>
        ...
        <ThemeText style={style.content}>
          ...
          <LinkText linkTo="https://mp.weixin.qq.com/s/sH_2vRm7EyBGgWggkJmsdg">这里</LinkText>
          ，是公众号链接，如果要了解后续更新的话，点个关注也行。
        </ThemeText>
        <ThemeText style={style.content}>
          本软件完全免费，并基于GPL协议开源，仅供学习参考使用，不可用于商业目的。代码地址如下，如果打不开试试把github换成gitee:
        </ThemeText>
        <LinkText linkTo={'https://github.com/maotoumao/MusicFree'}>https://github.com/maotoumao/MusicFree</LinkText>
        <ThemeText style={style.content}>
          PC 版本也已开源，代码地址如下：
          <LinkText linkTo={'https://github.com/maotoumao/MusicFreeDesktop'}>
        ...
        <ThemeText style={style.content}>
          <ThemeText fontWeight="bold">还请注意本软件只是个人的业余项目，距离正式版也有很长一段距离。</ThemeText>
          如果你在找成熟稳定的音乐软件，可以考虑其他优秀的软件。当然我会一直维护，让它变得尽可能的完善一些。业余时间用爱发电，进度慢还请见谅。
        </ThemeText>
        <ThemeText style={style.content}>
          如果是技术交流或者技术向的闲聊的话，欢迎加QQ群(Github Readme自取)，也可以直接发邮件;)
        </ThemeText>
