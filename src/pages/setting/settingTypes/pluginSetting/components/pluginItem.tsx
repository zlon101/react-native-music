        ...
        if (Array.isArray(plugin.instance.userVariables)) {
          showPanel('SetUserVariables', {
            async onOk(newValue, closePanel) {
              await PluginMeta.setPluginMetaProp(plugin, 'userVariables', newValue);
              Toast.success('设置成功~');
              closePanel();
            },
            variables: plugin.instance.userVariables,
            initValues: PluginMeta.getPluginMeta(plugin)?.userVariables,
          });
        }
      },
  ...
  return (
    <View
      ...
      <View style={styles.header}>
        <ThemeText style={styles.headerPluginName} numberOfLines={1} fontSize="title">
          {plugin.name}
        </ThemeText>
