...
export default function PluginSubscribe() {
  ...
  const onSubmit = (subscribeItem: ISubscribeItem, hideDialog: () => void, editingIndex?: number) => {
    if (subscribeItem.url.endsWith('.js') || subscribeItem.url.endsWith('.json')) {
      if (editingIndex !== undefined) {
        Config.set(
          'setting.plugin.subscribeUrl',
          JSON.stringify([...subscribes.slice(0, editingIndex), subscribeItem, ...subscribes.slice(editingIndex + 1)]),
        );
      } else {
        Config.set('setting.plugin.subscribeUrl', JSON.stringify([...subscribes, subscribeItem]));
      }
      hideDialog();
    } else {
                ...
                onPress={() => {
                  showDialog('SubscribePluginDialog', {
                    ...
                    onDelete(editingIndex, hideDialog) {
                      Config.set(
                        'setting.plugin.subscribeUrl',
                        JSON.stringify([...subscribes.slice(0, editingIndex), ...subscribes.slice(editingIndex + 1)]),
                      );
                      hideDialog();
                      Toast.success('删除成功');
                    },
