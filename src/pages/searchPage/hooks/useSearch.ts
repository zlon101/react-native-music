...
export default function useSearch() {
  ...
  const search = useCallback(
    async function (query?: string, queryPage?: number, type?: ICommon.SupportMediaType, pluginHash?: string) {
      /** 如果没有指定插件，就用所有插件搜索 */
      ...
      plugins.forEach(async plugin => {
        ...
        // 本次搜索关键词
        currentQueryRef.current = query = query ?? prevPluginResult?.query ?? '';
