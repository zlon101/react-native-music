...
interface IResultWrapperProps<T extends ICommon.SupportMediaType = ICommon.SupportMediaType> {
  tab: T;
  pluginHash: string;
  pluginName: string;
...
function ResultWrapper(props: IResultWrapperProps) {
  ...
  const [searchState, setSearchState] = useState<RequestStateCode>(searchResult?.state ?? RequestStateCode.IDLE);
  ...
  const renderItem = ({ item, index }: any) => (
    <ResultComponent item={item} index={index} pluginHash={pluginHash} pluginSearchResultRef={pluginSearchResultRef} />
  );
  ...
  return searchState === RequestStateCode.PENDING_FP ? (
    ...
    <FlashList
      ...
      onEndReached={() => {
        (searchState === RequestStateCode.PARTLY_DONE || searchState === RequestStateCode.IDLE) &&
          search(undefined, undefined, tab, pluginHash);
      }}
      estimatedItemSize={tab === 'sheet' ? rpx(306) : rpx(120)}
      numColumns={tab === 'sheet' ? (orientation === 'vertical' ? 3 : 4) : 1}
