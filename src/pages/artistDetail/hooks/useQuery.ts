...
export default function useQueryArtist(pluginHash: string) {
  ...
  const queryArtist = useCallback(
    async (artist: IArtist.IArtistItem, page?: number, type: IArtist.ArtistMediaType = 'music') => {
      const plugin = PluginManager.getByHash(pluginHash);

      const prevResult = queryResults[type];
      if (prevResult?.state === RequestStateCode.PENDING || prevResult?.state === RequestStateCode.FINISHED) {
        return;
      }
      ...
      try {
        ...
        setQueryResults(
          produce(draft => {
            ...
            draft[type].state = result?.isEnd === false ? RequestStateCode.PARTLY_DONE : RequestStateCode.FINISHED;
