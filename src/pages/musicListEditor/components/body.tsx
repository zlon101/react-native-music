  ...
  return (
    <HorizonalSafeAreaView style={globalStyle.flex1}>
      <View style={style.header}>
        <Button
          onPress={() => {
            if (selectedItems.length !== editingMusicList.length && editingMusicList.length) {
              setEditingMusicList(
          ...
          {`${selectedItems.length !== editingMusicList.length && editingMusicList.length ? '全选' : '全不选'} (已选${
            selectedItems.length
          }首)`}
        </Button>
        <Button
          fontColor={musicListChanged && musicSheet?.id ? 'text' : 'textSecondary'}
          onPress={async () => {
            if (musicListChanged && musicSheet?.id) {
              if (musicSheet.id === localMusicSheetId) {
                ...
                await MusicSheet.updateAndSaveSheet(musicSheet.id, {
                  musicList: editingMusicList.map(_ => _.musicItem),
                });
              }
