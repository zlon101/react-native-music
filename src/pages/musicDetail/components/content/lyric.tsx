...
import { DeviceEventEmitter, LayoutRectangle, StyleSheet, Text, View } from 'react-native';
...
function useLyric() {
  ...
  async function refreshLyric() {
    setLoading(true);
    try {
      const lrc = await PluginManager.getByMedia(musicItem)?.methods?.getLyricText(musicItem);
      setLoading(false);
  ...
  useEffect(() => {
    if (!lrcManagerRef.current || !isSameMediaItem(lrcManagerRef.current?.getCurrentMusicItem?.(), musicItem)) {
      refreshLyric();
    }
  ...
  useEffect(() => {
    const unsubscription = DeviceEventEmitter.addListener(EDeviceEvents.REFRESH_LYRIC, refreshLyric);
    return () => {
...
export default function Lyric() {
  ...
  const [draggingIndex, setDraggingIndex, setDraggingIndexImmi] = useDelayFalsy<number | undefined>(undefined, 2000);
  ...
  const onScroll = (e: any) => {
    if (drag) {
      setDraggingIndex(Math.min(Math.floor(e.nativeEvent.contentOffset.y / ITEM_HEIGHT), lyric.length - 1));
    }
  };
      ...
      {draggingIndex !== undefined && (
        <View
          ...
          <Text style={style.draggingTimeText}>
            {timeformat(Math.min((lyric[draggingIndex]?.time ?? 0) + (meta?.offset ?? 0), progress.duration ?? 0))}
          </Text>
          <View style={style.singleLine} />

          <IconButtonWithGesture style={style.playIcon} sizeType="small" name="play" onPress={onLyricSeekPress} />
        </View>
      )}
    </View>
  );
