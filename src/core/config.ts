...
type KeyPathValue<T extends object, K extends string> = T extends Record<string | number, any>
  ? K extends `${infer S}.${infer R}`
    ? KeyPathValue<T[S], R>
    : T[K]
  : never;

type KeyPathsObj<T extends object, K extends string = KeyPaths<T>> = T extends Record<string | number, any>
  ? {
...
/** 设置config */
async function setConfig<T extends IConfigPaths>(key: T, value: IConfigPathsObj[T], shouldNotify = true) {
  if (config === null) {
    return;
  }
