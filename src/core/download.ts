...
const downloadingQueueStateMapper = new StateMapper(() => downloadingMusicQueue);
const pendingMusicQueueStateMapper = new StateMapper(() => pendingMusicQueue);
const downloadingProgressStateMapper = new StateMapper(() => downloadingProgress);
...
const getDownloadPath = (fileName?: string) => {
  const dlPath = Config.get('setting.basic.downloadPath') ?? pathConst.downloadMusicPath;
  if (!dlPath.endsWith('/')) {
...
function removeFromPendingQueue(item: IDownloadMusicOptions) {
  const targetIndex = pendingMusicQueue.findIndex(_ => isSameMediaItem(_.musicItem, item.musicItem));
  if (targetIndex !== -1) {
    pendingMusicQueue = pendingMusicQueue.slice(0, targetIndex).concat(pendingMusicQueue.slice(targetIndex + 1));
...
function removeFromDownloadingQueue(item: IDownloadMusicOptions) {
  const targetIndex = downloadingMusicQueue.findIndex(_ => isSameMediaItem(_.musicItem, item.musicItem));
  if (targetIndex !== -1) {
...
function generateFilename(musicItem: IMusic.IMusicItem) {
  return `${escapeCharacter(musicItem.platform)}@${escapeCharacter(musicItem.id)}@${escapeCharacter(
    musicItem.title,
  )}@${escapeCharacter(musicItem.artist)}`.slice(0, 200);
}
...
async function downloadNext() {
  ...
  try {
    if (plugin) {
      const qualityOrder = getQualityOrder(
        quality ?? Config.get('setting.basic.defaultDownloadQuality') ?? 'standard',
  ...
  /** 目标下载地址 */
  const targetDownloadPath = addFileScheme(getDownloadPath(`${nextDownloadItem.filename}.${extension}`));
...
/** 加入下载队列 */
function downloadMusic(musicItems: IMusic.IMusicItem | IMusic.IMusicItem[], quality?: IMusic.IQualityKey) {
  if (Network.isOffline()) {
    Toast.warn('当前无网络，无法下载');
    return;
  }
  if (Network.isCellular() && !Config.get('setting.basic.useCelluarNetworkDownload')) {
  ...
  musicItems = musicItems.filter(
    musicItem =>
      pendingMusicQueue.findIndex(_ => isSameMediaItem(_.musicItem, musicItem)) === -1 &&
      downloadingMusicQueue.findIndex(_ => isSameMediaItem(_.musicItem, musicItem)) === -1 &&
