  ...
  try {
    ...
    if (isSameMediaItem(musicItem, realtimeMusicItem)) {
      if (lrc) {
        ...
        // 更新当前状态的歌词
        const currentLyric = fromStart ? parser.getLyric()[0] : parser.getPosition(await MusicQueue.getPosition()).lrc;
...
async function setup() {
  ...
  if (Config.get('setting.lyric.showStatusBarLyric')) {
    // LyricUtil.showStatusBarLyric('MusicFree', Config.get('setting.lyric') ?? {});
  }
