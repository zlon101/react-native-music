...
async function resumeSheets(sheets: ICommon.WithMusicList<IMusic.IMusicSheetItemBase>[]) {
  ...
  for (let i = 0; i < sheets.length; ++i) {
    ...
    if (musicSheet.id === 'favorite') {
      ...
      newSheetMusicMap[musicSheet.id] = originalMusicList.concat(
        musicSheet.musicList?.filter(item => originalMusicList.findIndex(_ => isSameMediaItem(_, item)) === -1) ?? [],
      );
    } else {
      const newId = nanoid();
      ...
      newSheets = [
        ...newSheets,
        {
          ...
          coverImg: musicSheet.coverImg?.startsWith('http')
            ? musicSheet.coverImg
            : (musicSheet.musicList ?? [])[(musicSheet.musicList?.length ?? 0) - 1]?.artwork,
        },
      ];
  ...
  await Promise.all(needUpdatedIds.map(k => setStorage(k, newSheetMusicMap[k] ?? [])));
...
async function addMusic(sheetId: string, musicItem: IMusic.IMusicItem | Array<IMusic.IMusicItem>) {
  if (!Array.isArray(musicItem)) {
    musicItem = [musicItem];
  }
  const musicList = sheetMusicMap[sheetId] ?? [];
  musicItem = musicItem.filter(item => musicList.findIndex(_ => isSameMediaItem(_, item)) === -1);
  const newMusicList = musicList.concat(musicItem);
  let basic;
  if (!musicSheets.find(_ => _.id === sheetId)?.coverImg?.startsWith('file://')) {
    basic = {
...
async function removeMusic(sheetId: string, musicItems: IMusic.IMusicItem | IMusic.IMusicItem[]) {
