...
export async function addMusic(musicItem: IMusic.IMusicItem | IMusic.IMusicItem[]) {
...
export async function removeMusic(musicItem: IMusic.IMusicItem, deleteOriginalFile = false) {
  const idx = localSheet.findIndex(_ => isSameMediaItem(_, musicItem));
  let newSheet = [...localSheet];
  if (idx !== -1) {
    const localMusicItem = localSheet[idx];
    newSheet.splice(idx, 1);
    const localPath = musicItem[internalSerializeKey]?.localPath ?? localMusicItem[internalSerializeKey]?.localPath;
...
let metas: any[] = [];
  ...
  const musicItems = await Promise.all(
    musicList.map(async (musicStat, index) => {
      let { platform, id, title, artist } = parseFilename(musicStat.filename) ?? {};
...
/** 是否为本地音乐 */
function isLocalMusic(musicItem: ICommon.IMediaBase | null): IMusic.IMusicItem | undefined {
  return musicItem ? localSheet.find(_ => isSameMediaItem(_, musicItem)) : undefined;
}
