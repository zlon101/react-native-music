import { copyFile, exists, readDir, readFile, unlink, writeFile } from 'react-native-fs';
...
export class Plugin {
  ...
  constructor(funcCode: string | (() => IPlugin.IPluginInstance), pluginPath: string) {
    this.state = 'enabled';
    ...
    try {
      if (typeof funcCode === 'string') {
        ...
        const env = {
          getUserVariables: () => {
            return PluginMeta.getPluginMeta(this)?.userVariables ?? {};
          },
          os: 'android',
        };
  ...
  private checkValid(_instance: IPlugin.IPluginInstance) {
    /** 版本号校验 */
    if (_instance.appVersion && !satisfies(DeviceInfo.getVersion(), _instance.appVersion)) {
      throw {
        instance: _instance,
...
class PluginMethods implements IPlugin.IPluginInstanceMethods {
  ...
  async getMediaSource(
    ...
    try {
      const { url, headers } = (await this.plugin.instance.getMediaSource(musicItem, quality)) ?? {
  ...
  /** 获取音乐详情 */
  async getMusicInfo(musicItem: ICommon.IMediaBase): Promise<Partial<IMusic.IMusicItem> | null> {
  ...
  /** 获取歌词 */
  async getLyric(musicItem: IMusic.IMusicItemBase, from?: IMusic.IMusicItemBase): Promise<ILyric.ILyricSource | null> {
    // 1.额外存储的meta信息
    const meta = MediaMeta.get(musicItem);
    if (meta && meta.associatedLrc) {
      // 有关联歌词
      if (isSameMediaItem(musicItem, from) || isSameMediaItem(meta.associatedLrc, musicItem)) {
        // 形成环路，断开当前的环
      ...
      const result = await this.getLyric({ ...meta.associatedLrc, ...associatedMeta }, from ?? musicItem);
      if (result) {
    ...
    // 2.本地缓存
    const localLrc = meta?.[internalSerializeKey]?.local?.localLrc || cache?.[internalSerializeKey]?.local?.localLrc;
    ...
    if (!lrcUrl) {
      ...
      try {
        ...
        if (from) {
          ...
          lrcSource = await this.plugin.instance?.getLyric?.(resetMediaItem(musicItem, undefined, true));
        }
    ...
    if (rawLrc || lrcUrl) {
      ...
      if (rawLrc) {
        ...
        // 写入缓存
        Cache.update(musicItem, [[`${internalSerializeKey}.local.localLrc`, filename]]);
        // 如果有meta
        if (meta) {
          MediaMeta.update(musicItem, [[`${internalSerializeKey}.local.localLrc`, filename]]);
        }
        return {
          rawLrc,
          lrc: lrcUrl,
  ...
  /** 获取歌词文本 */
  async getLyricText(musicItem: IMusic.IMusicItem): Promise<string | undefined> {
    return (await this.getLyric(musicItem))?.rawLrc;
  }

  /** 获取专辑信息 */
  async getAlbumInfo(albumItem: IAlbum.IAlbumItemBase, page: number = 1): Promise<IPlugin.IAlbumInfoResult | null> {
  ...
  async getTopListDetail(
    ...
    try {
      const result = await this.plugin.instance?.getTopListDetail?.(topListItem);
      if (!result) {
        throw new Error();
      }
      if (result.musicList) {
...
// 安装插件
async function installPlugin(pluginPath: string, config?: IInstallPluginConfig) {
...
async function installPluginFromUrl(url: string, config?: IInstallPluginConfig) {
  try {
...
function getByHash(hash: string) {
  return hash === localPluginHash ? localFilePlugin : plugins.find(_ => _.hash === hash);
}
...
function getSortedSearchablePlugins(supportedSearchType?: ICommon.SupportMediaType) {
  return getSearchablePlugins(supportedSearchType).sort((a, b) =>
    (PluginMeta.getPluginMeta(a).order ?? Infinity) - (PluginMeta.getPluginMeta(b).order ?? Infinity) < 0 ? -1 : 1,
  );
}
...
function getRecommendSheetablePlugins() {
  return plugins.filter(_ => _.state === 'enabled' && _.instance.getRecommendSheetsByTag);
}
...
function useSortedPlugins() {
  ...
  const [sortedPlugins, setSortedPlugins] = useState(
    [..._plugins].sort((a, b) =>
      (_pluginMetaAll[a.name]?.order ?? Infinity) - (_pluginMetaAll[b.name]?.order ?? Infinity) < 0 ? -1 : 1,
    ),
  );
