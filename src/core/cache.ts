...
const cache = new LRUCache<string, ICommon.IMediaMeta>({
  ...
  dispose: async (value, key) => {
    ...
    if (localFiles) {
      const localMeta = MediaMeta.getByMediaKey(key)?.[internalSerializeKey]?.local;
...
function getCache(mediaItem: ICommon.IMediaBase): ICommon.IMediaMeta | undefined {
  // sync
...
function getCacheInternal(mediaItem: ICommon.IMediaBase): Record<string, any> | undefined {
...
function updateCache(mediaItem: ICommon.IMediaBase, patch: ICommon.IMediaMeta | Array<[string, any]>) {
